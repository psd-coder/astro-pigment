...
If your host runs edge middleware, add a one-line entry file so agents that request a page get its `.md` twin at the same URL — a `200` with `Content-Type: text/markdown`, no client redirect. Vercel, Netlify, Cloudflare Pages, and custom Node/Bun servers are each supported — see [Negotiating markdown for AI agents](/api#negotiating-markdown-for-ai-agents) in the API reference for the per-host setup.
